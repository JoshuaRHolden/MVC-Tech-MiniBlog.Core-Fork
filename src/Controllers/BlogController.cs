...
namespace Miniblog.Core.Controllers
{
    public class BlogController : Controller
    {
        ...
        public async Task<IActionResult> Index([FromRoute]int page = 0)
        {
            ...
            ViewData["SiteHeader"] = "MVC Tech Blog";
            // set the view option

            ViewData["AllCategories"] = categories.ToList().OrderBy(x=>x.ToString());
        ...
        [Route("/blog/search/{srcString}/{page:int?}")]
        [OutputCache(Profile = "default")]
        public async Task<IActionResult> Search(string srcString, int page = 0)
        {
            // get posts for the selected category.
            var posts = await _blog.GetPostsBySearchString(srcString);
            ViewData["SiteHeader"] = "MVC Tech Blog";
            // apply paging filter.
            var filteredPosts = posts.Skip(_settings.Value.PostsPerPage * page).Take(_settings.Value.PostsPerPage);
            ...
            ViewData["TotalPostCount"] = posts.Count();
            ViewData["Title"] = _manifest.Name + " " + srcString;
            ViewData["Description"] = $"Articles posted matching search: {srcString}";
            ViewData["prev"] = $"/blog/search/{srcString}/{page + 1}/";
            ViewData["next"] = $"/blog/search/{srcString}/{(page <= 1 ? null : page - 1 + "/")}";
            return View("~/Views/Blog/Index.cshtml", filteredPosts);
        }
        ...
        public async Task<IActionResult> Post(string slug)
        {
            ...
            ViewData["SiteHeader"] = post.Title;
            var categories = await _blog.GetCategories();
            ViewData["AllCategories"] = categories.ToList().OrderBy(x => x.ToString());
            if (post != null)
            {
        ...
        public async Task<IActionResult> UpdatePost(Post post)
        {
            ...
            var cats = await _blog.GetCategories();
            ViewData["AllCategories"] = cats.ToList().OrderBy(x => x.ToString());
